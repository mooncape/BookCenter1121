 ...
 *= require mercury/locales/zh.local
 *
  ...
  config: {
    ...
    toolbars: {
      primary: {
        ...
        comment:               ['Comment', "Add some comments"],
        commentVideo:          ['Video', "Add some comments"],
        commentAudio:          ['Audio', "Add some comments"],
        ...
        upload:                ['Upload', 'Upload Videos and Audios', { panel: '/mercury/modals/upload.html' }]
      ...
      editable: {
        ...
        predefined:            {
          ...
          //sep1:                ' ',
          fontsize:            ['Fontsize', null, { select: '/mercury/selects/fontsize.html', preload: true }],
          sep2:                ' ',
          fontname:            ['Fontname', null, { select: '/mercury/selects/fontname.html', preload: true }],
          sep3:                ' ',
    ...
    uploading: {
      enabled: true,
      allowedMimeTypes: ['image/jpeg', 'image/gif', 'image/png', 'video/mp4', 'application/pdf', 'text/plain'],
    ...
    uploadingVideo: {
      enabled: true,
      allowedMimeTypes: ['image/jpeg', 'image/gif', 'image/png', 'video/mp4'],
      maxFileSize: 1235242880,
      inputName: 'image[image]',
      url: '/mercury/images',
      handler: false
    ...
    behaviors: {
      ...
      comment: function(selection) {
        //var value = prompt('Enter the link for the button');
        var date = new Date();
        var divID = date.getTime();
        var string = '<div><input type="button"  style=" width:10px; height:10px;  border:0; background:url(/comment.png) no-repeat left top" onClick=\'var com=document.getElementById("' + divID;
        string += '").style;if(com.display=="none"){com.display="block";}else{com.display="none";}\'></input><details id="' + divID;
        string += '" style="color:blue"><summary>注释</summary><p>' + '内容' + '</p></details></div>';
        selection.replace(string);
        // selection.replace('<div><h2>Title</h2><p>content</p><a href="' + value + '" class="button">button</a><hr/></div>');
      },
      commentVideo: function(selection) {
        //var value = prompt('Enter the link for the button');
        var date = new Date();
        var divID = date.getTime();
        var string = '<div><input type="button"  style=" width:9px; height:10px;  border:0; background:url(/videocomment.png) no-repeat left top" onClick=\'var com=document.getElementById("' + divID;
        string += '").style;if(com.display=="none"){com.display="block";}else{com.display="none";}\'></input><details id="' + divID;
        string += '" style="color:blue"><summary>注释</summary><p>' + '内容' + '</p></details></div>';
        selection.replace(string);
        // selection.replace('<div><h2>Title</h2><p>content</p><a href="' + value + '" class="button">button</a><hr/></div>');
      },
      commentAudio: function(selection) {
        //var value = prompt('Enter the link for the button');
        var date = new Date();
        var divID = date.getTime();
        var string = '<div><input type="button"  style=" width:10px; height:10px;  border:0; background:url(/audiocomment.png) no-repeat left top" onClick=\'var com=document.getElementById("' + divID;
        string += '").style;if(com.display=="none"){com.display="block";}else{com.display="none";}\'></input><details id="' + divID;
        string += '" style="color:blue"><summary>注释</summary><p>' + '内容' + '</p></details></div>';
        selection.replace(string);
        // selection.replace('<div><h2>Title</h2><p>content</p><a href="' + value + '" class="button">button</a><hr/></div>');
...
jQuery(window).on('mercury:ready', function() { 
  ...
  var prevlink = $('#mercury_iframe').contents().find('#prew_page');
  var nextlink = $('#mercury_iframe').contents().find('#next_page');

  Mercury.saveURL = link.data('save-url');

  console.log(link + " " + prevlink + " " + nextlink)
...
// Event.observe(window, 'mercury:saved', function() {
//   window.location = window.location.href.replace(/\/editor\//i, '/');
//   link.show();
// });

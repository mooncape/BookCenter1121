@Mercury.modalHandlers.insertMedia = {
  ...
  submitForm: ->
    ...
    switch type
      when 'image_url'
        img_src = @element.find('#media_image_url').val().replace(/\/(original)?(xlarge)?(large)?(medium)?(small)?\//, "/#{@element.find('#media_image_resize').val()}/")
      ...
      when 'youtube_url'
        ...
        value = '<video width="' + parseInt(@element.find('#media_youtube_width').val(), 10) || 560
        value += '" height="' + parseInt(@element.find('#media_youtube_height').val(), 10) || 349
        value += '" controls="controls" src="/' + @element.find('#media_video_src').val()
        value += '" ></video>'
      ...
      when 'vimeo_url'
        ...
        value = '<audio controls="controls" src="/' + @element.find('#media_audio_src').val()
        value += '"></audio>'
